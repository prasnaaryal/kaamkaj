...
const Banner = ({query,handleInputChange}) => {
  ...
  return (
    <div className='max-w-screen-2xl container mx-auto xl:px-24 px-4 md:py-20 py-14'>
        ...
        <p className='text-lg text-black/70 mb-8'>Thousands of jobs in the computer,engineering and technology</p>
        <form>
            <div className='flex justify-start md:flex-row flex-col md:gap-0 gap-4'>
                ...
                focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 md:w-1/2 w-full'>
                    <input type="text" name="title" id="title" placeholder="What position are you looking for?"
                     className='block flex-1 border-0 bg-transparent py-1.5 pl-8 text-gray-900
                      placeholder:text-gray-400 focus:right-0 sm:text-sm sm:leading-6' onChange={handleInputChange}
                        value={query}/>
                ...
                <button type="submit" className='bg-blue-500 text-white py-2 px-8 md:rounded-s-none rounded'>Search</button>
            </div>
        </form>
